...
public class Genome {
    ...
    public Genome(int n, Animal parent1, Animal parent2){
        ...
        int parent1Share = parent1.getEnergy() / combinedEnergy;
        int parent2Share = parent2.getEnergy() / combinedEnergy;

        if(rand){
            int howManyIdxParent1 = parent1Share * parent1.getGenome().size();
            ArrayList<Integer> partOfParent1 = (ArrayList<Integer>) parent1.getGenome().subList(0, howManyIdxParent1);

            int howManyIdxParent2 = parent2Share * parent2.getGenome().size();
